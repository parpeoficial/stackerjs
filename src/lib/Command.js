...
export class Command 
{
    ...
    getAutoLoad() 
    {
        return existsSync(this.autoloadPath) ?
            JSON.parse(readFileSync(this.autoloadPath, { encoding: "utf8" })) : { commands: {} };
    }
    ...
    loadSample(name) 
    {
        return existsSync(`${process.cwd()}/node_modules/stackerjs/resources/samples/sample.${name}`) ?
            readFileSync(`${process.cwd()}/node_modules/stackerjs/resources/samples/sample.${name}`, { encoding: "utf8" }) :
            readFileSync(`${process.cwd()}/resources/samples/sample.${name}`, { encoding: "utf8" });
    }
