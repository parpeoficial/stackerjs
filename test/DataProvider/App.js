...
microService.setRoute("get", "/", () => "Hello world");
microService.setRoute("get", "/hi-promise", () => 
{
    return new Promise(resolve => setTimeout(() => resolve("In Promise"), 500));
});
microService.setRoute("put", "/person/:id", request =>
    new Http.Response().setStatusCode(Http.Response.HTTP_OK).setContent(request.get("id") > 5
        ? "OK"
        : {
            status: true,
            url: {
...
microService.setRoute("patch", "/person/:id", request =>
    new Http.Response().setStatusCode(Http.Response.HTTP_OK).setContent(request.get("id") > 5
        ? "OK"
        : {
            status: true,
